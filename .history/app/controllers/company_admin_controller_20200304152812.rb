...
class CompanyAdminController < ApplicationController
  ...
  def update
    @service_center_admin = User.find(params[:id])
    @service_center_admin.update(firstname: params[:user][:firstname], lastname: params[:user][:lastname], username: params[:user][:username], contact: params[:user][:contact], email: params[:user][:eamil])
  end
